    ...
    if adjoint:
        ...
        for tt in range(kp):
            for ch in range(kc):
                ...
                res[:,:,ch,tt] = np.reshape(nufft_adj(b.flatten('F'), a['st']['phase_%d'%(tt)])/np.sqrt(a['imSize'].prod()),(kx,ky),order='F')
        ...
        for tt in range(kp):
            for ch in range(kc):
                ...
                ress[:,:,ch,tt] = np.multiply(np.reshape(nufft(res,a['st']['phase_%d'%(tt)])/np.sqrt(a['imSize'].prod()),(a['dataSize'][0][0],a['dataSize'][0][1]),order='F'), a['w'][:,:,tt])
    return ress
...
def nufft_adj(X, st):
    ...
    Xk_all = stp.dot(X)#stp.real.dot(X.real) + stp.imag.dot(X.imag)
